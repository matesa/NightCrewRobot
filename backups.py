...
def import_data(update, context):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            update.effective_message.reply_text("Bu yalnızca grup komutudur!")
            return ""
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        try:
            ...
            msg.reply_text(
                "Dosyayı tekrar indirmeyi ve yüklemeyi deneyin, bu bana bozuk görünüyor!"
            )
            return
        ...
        if len(data) > 1 and str(chat.id) not in data:
            msg.reply_text(
                "Bu dosyada birden fazla grup var ve chat.id aynı değil! Onu nasıl içe aktaracağım?"
            )
            return
        ...
        try:
            if data.get(str(chat.id)) is None:
                if conn:
                    text = "Yedekleme başka bir sohbetten geliyor, başka bir sohbeti sohbete geri dönemem *{}*".format(
                        chat_name
                    )
                else:
                    text = "Yedek başka bir sohbetten geliyor, bu sohbete başka bir sohbeti geri veremiyorum"
                return msg.reply_text(text, parse_mode="markdown")
        except Exception:
            return msg.reply_text("Verileri içe aktarırken bir sorun oluştu!")
        # Check if backup is from self
        try:
            if str(context.bot.id) != str(data[str(chat.id)]["bot"]):
                return msg.reply_text(
                    "Önerilmeyen başka bir bottan yedekleme, soruna neden olabilir, belgeler, fotoğraflar, videolar, sesler, kayıtlar olması gerektiği gibi çalışmayabilir."
                )
        except Exception:
            pass
        # Select data source
        ...
        try:
            ...
            msg.reply_text(
                f"Verilerinizi kurtarırken bir hata oluştu. İşlem başarısız oldu. Bununla ilgili bir sorun yaşarsanız, lütfen @{SUPPORT_CHAT} adresine götürün"
            )

            LOGGER.exception(
                " %s adlı %s sohbeti için gösterim başarısız oldu.",
                str(chat.id),
                str(chat.title),
            )
            return
        ...
        if conn:
            ...
            text = "Yedekleme, *{}* tarihinde tamamen geri yüklendi.".format(chat_name)
        else:
            text = "Yedekleme tamamen geri yüklendi"
...
def export_data(update, context):
    ...
    if checkchat.get("status"):
        if jam <= int(checkchat.get("value")):
            ...
            update.effective_message.reply_text(
                "Yalnızca günde bir kez yedekleme yapabilirsiniz!\nYaklaşık `{}` sonra yeniden yedekleme yapabilirsiniz.".format(
                    timeformatt
                ),
    ...
    try:
        context.bot.sendMessage(
            JOIN_LOGGER,
            "*Başarıyla içe aktarıldı yedekleme:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`".format(
...
__help__ = """
*Yalnızca grup sahibi için:*

 • /import: Butler / emilia grubunun mümkün olduğunca fazla içe aktarması için yedekleme dosyasını yanıtlayın, bu da transferleri çok kolaylaştırır! \
 Telegram kısıtlamaları nedeniyle dosyaların / fotoğrafların alınamayacağını unutmayın.
 • /export: Dışa aktarılacak olan grup verilerini dışa aktar: kurallar, notlar (belgeler, resimler, müzik, video, ses, ses, metin, metin düğmeleri) \
