...
def add_chat(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        msg.reply_text("PM 'da Chat Botu etkinleştiremezsiniz.")
        return
    ...
    if not is_chat:
        ...
        msg.reply_text("Chat Bot bu sohbet için başarıyla etkinleştirildi!")
        message = (
        ...
        msg.reply_text("Chat Bot bu sohbet için zaten etkinleştirildi!")
        return ""
...
def remove_chat(update: Update, context: CallbackContext):
    ...
    is_chat = sql.is_chat(chat.id)
    if not is_chat:
        msg.reply_text("Chat Bot burada ilk etapta etkinleştirilmemiştir!")
        return ""
    else:
        sql.rem_chat(chat.id)
        msg.reply_text("Chat Bot başarıyla devre dışı bırakıldı!")
        message = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#AI_DISABLED\n"
            f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
__help__ = f"""
Chatbot, CoffeeHouse API'sini kullanır ve NightCrewRobot'un konuşmasına izin verir ve daha etkileşimli bir grup sohbeti deneyimi sağlar.
...
@{SUPPORT_CHAT} adresindeki hataları bildirir
"""
