        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    text = " {} içindeki yönetici olmayanlar için {} kilitlendi!".format(
                        ltype, chat_name)
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                            update.effective_message,
                            "TBu komutun PM'de olmayan grupta kullanılması amaçlanmıştır",
                        )
                        return ""
                    chat = update.effective_chat
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = "Yönetici olmayanlar için {} Kilitli!".format(ltype)
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = " {} içindeki tüm yönetici olmayanlar için {} Kilitli!".format(
                        ltype, chat_name)
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                            update.effective_message,
                            "Bu komutun PM'de olmayan grupta kullanılması amaçlanmıştır",
                        )
                        return ""
                    chat = update.effective_chat
                    chat_id = update.effective_chat.id
                ...
                send_message(
                    update.effective_message,
                    "Neyi kilitlemeye çalışıyorsunuz...?  Kilitlenebilirler listesi için /locktypes türlerini deneyin",
                )
        else:
            send_message(update.effective_message,
                         "Neyi kilitlemeye çalışıyorsun...?")

    else:
        send_message(
            update.effective_message,
            "Yönetici değilim veya yeterli haklara sahip değilim.",
        )
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = " {} içindeki herkes için {} kilidi açıldı!".format(
                        ltype, chat_name)
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = "Herkes için {} kilidi açıldı!".format(ltype)
                ...
                if not can_change_info:
                    send_message(
                        update.effective_message,
                        "Grup bilgilerini değiştirme iznim yok.",
                        parse_mode="markdown",
                    )
                    return
                ...
                send_message(
                    update.effective_message,
                    "Neyin kilidini açmaya çalışıyorsunuz...? Kilitlenebilirler listesi için /locktypes deneyin.",
                )

        else:
            send_message(update.effective_message,
                         "Neyin kilidini açmaya çalışıyorsun...?")
                    ...
                    if "ARABIC" in check:
                        try:
                            ...
                            if excp.message == "Silinecek mesaj bulunamadı":
                                ...
                                LOGGER.exception("kilitlenebilirlerde HATA")
                        break
                if message.text:
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, context.bot.id):
                            send_message(
                                update.effective_message,
                                "Bir bot görüyorum ve onlara katılmalarını engellemem söylendi..."
                        ...
                        send_message(
                            update.effective_message,
                            "Bu sohbete yalnızca yöneticiler bot ekleyebilir! Buradan çıkın.",
                        )
                        break
            else:
                try:
...
def build_lock_message(chat_id):
    ...
    permslist = []
    if locks:
        res += "*" + "Bunlar, Sohbet'teki mevcut kilitlerdir:" + "*"
        if locks:
    ...
    if locklist:
        ...
        for x in locklist:
            res += "\n • {}".format(x)
    res += "\n\n*" + "Bunlar mevcut sohbet izinleridir:" + "*"
    for x in permslist:
        res += "\n • {}".format(x)
    return res
...
def list_locks(update, context):
    ...
    if conn:
        res = res.replace("Kilitlenir", "*{}*".format(chat_name))
...
__help__ = """
Do stickers Çıkartmalar sizi rahatsız ediyor mu? veya insanların bağlantı paylaşmasını önlemek mi istiyorsunuz? veya resimler? \
Doğru yerdesiniz!
Kilit modülü,deki bazı ortak öğeleri kilitlemenizi sağlar
telegram dünyası; bot onları otomatik olarak silecek!

 • `/locktypes`*:* Olası tüm kilit türlerini listeler
 
*Yalnızca yöneticiler:*
 • `/lock <type>`*:* Belirli bir türdeki öğeleri kilitle (özel olarak kullanılamaz)
 • `/unlock <type>`*:* Belirli bir türdeki öğelerin kilidini açın (özel olarak kullanılamaz)
 • `/locks`*:* Bu sohbetteki mevcut kilit listesi.
 
Kilitler, bir grubun kullanıcılarını kısıtlamak için kullanılabilir.
Örneğin:
Kilitleme URL'ler tüm sınırlar çıkartmaları kilitleme, adresler ile tüm iletileri otomatik silecektir \
yönetici olmayan kullanıcıların çıkartma vb. göndermesini engelleyin.
Botları kilitlemek, yönetici olmayanların sohbete bot eklemesini durdurur.

*Not:*
 • Kilit açma izni *info* üyelerin (yönetici olmayanların) açıklama veya grup adı gibi grup bilgilerini değiştirmesine izin verir
 • Kilit açma izni *pin* üyelerin (yönetici olmayanların) bir gruptaki bir mesajı sabitlemesine izin verir
"""
