...
def show_url(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            feed_title = link_processed.feed.get("title", default="Bilinmeyen")
            feed_description = "<i>{}</i>".format(
                re.sub(
                    '<[^<]+?>', '',
                    link_processed.feed.get("description", default="Bilinmeyen")))
            feed_link = link_processed.feed.get("link", default="Bilinmeyen")

            feed_message = "<b>Yayın Başlığı:</b> \n{}" \
                           "\n\n<b>Yayın Açıklaması:</b> \n{}" \
                           "\n\n<b>Yayın Bağlantısı:</b> \n{}".format(html.escape(feed_title),
                                                               feed_description,
                ...
                entry_description = "<i>{}</i>".format(
                    re.sub(
                        '<[^<]+?>', '', link_processed.entries[0].get(
                            "description", default="Bilinmeyen")))
                entry_link = link_processed.entries[0].get(
                    "link", default="Bilinmeyen")
            ...
            update.effective_message.reply_text(
                "Bu bağlantı bir RSS Besleme bağlantısı değildir")
    else:
...
def list_urls(update: Update, context: CallbackContext):
    ...
    if len(final_content) == 0:
        bot.send_message(
            chat_id=tg_chat_id, text="Bu sohbet hiçbir bağlantıya abone değil")
    elif len(final_content) <= constants.MAX_MESSAGE_LENGTH:
        bot.send_message(
            chat_id=tg_chat_id,
            text="Bu sohbet şu bağlantılara abone:\n" +
            final_content)
    else:
        bot.send_message(
            chat_id=tg_chat_id,
    ...
    for row in user_data:
        ...
        if len(new_entry_links) < 5:
            ...
            bot.send_message(
                chat_id=tg_chat_id,
                parse_mode=ParseMode.HTML,
                text="<b>Uyarı: </b>{} spam önlemek için tekrar atıldı"
...
__help__ = """
 ...
 • `/listrss`*:* sohbetin o anda abone olduğu rss beslemelerinin listesini gösterir.

*NOT:* Gruplarda, yalnızca yöneticiler grubun aboneliğine RSS bağlantıları ekleyebilir / kaldırabilir
"""
