...
def ping(update: Update, context: CallbackContext):
    ...
    start_time = time.time()
    message = msg.reply_text("Ping atılıyor...")
    end_time = time.time()
    ...
    message.edit_text(
        "PONG!!\n"
        "<b>Geçen Süre:</b> <code>{}</code>\n"
        "<b>Hizmet çalışma süresi:</b> <code>{}</code>".format(telegram_ping, uptime),
...
def pingall(update: Update, context: CallbackContext):
    ...
    reply_msg = "⏱Ping sonuçları:\n"
    reply_msg += "\n".join(pinged_list)
    reply_msg += '\n<b>Hizmet çalışma süresi:</b> <code>{}</code>'.format(uptime)
