...
def speedtestxyz(update: Update, context: CallbackContext):
    ...
    update.effective_message.reply_text(
        "SpeedTest Modunu Seçin", reply_markup=InlineKeyboardMarkup(buttons))
...
def speedtestxyz_callback(update: Update, context: CallbackContext):
    ...
    if query.from_user.id in DEV_USERS:
        msg = update.effective_message.edit_text('Hızlı test çalıştırılıyor....')
        speed = speedtest.Speedtest()
        speed.get_best_server()
        speed.download()
        speed.upload()
        replymsg = 'SpeedTest Sonuçları:'
        ...
        query.answer(
            "Bu komutu kullanmak için Kahramanlar Derneği'ne katılmanız gerekiyor.")
