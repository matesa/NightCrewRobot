...
if is_module_loaded(FILENAME):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text(
                    f"`{disable_cmd}` kullanımını devre dışı bıraktı",
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if disabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"`{disabled_cmds_string}` kullanımlarını devre dışı bıraktı",
                    parse_mode=ParseMode.MARKDOWN)

            if failed_disabled_cmds:
                failed_disabled_cmds_string = ", ".join(failed_disabled_cmds)
                update.effective_message.reply_text(
                    f"`{failed_disabled_cmds_string}` komutları devre dışı bırakılamaz",
                    parse_mode=ParseMode.MARKDOWN)
    ...
    def enable(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text(
                    f"`{enable_cmd}` kullanımını etkinleştirdi",
                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Bu bile devre dışı mı?")

        else:
            update.effective_message.reply_text("Neyi etkinleştirmeliyim?")
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if enabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"`{enabled_cmds_string}` kullanımlarını etkinleştirdi",
                    parse_mode=ParseMode.MARKDOWN)

            if failed_enabled_cmds:
                failed_enabled_cmds_string = ", ".join(failed_enabled_cmds)
                update.effective_message.reply_text(
                    f"`{failed_enabled_cmds_string}` komutları devre dışı bile mi?",
                    parse_mode=ParseMode.MARKDOWN)
    ...
    def list_cmds(update: Update, context: CallbackContext):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text(
                f"Aşağıdaki komutlar değiştirilebilir:\n{result}",
                parse_mode=ParseMode.MARKDOWN)
        else:
    ...
    def build_curr_disabled(chat_id: Union[str, int]) -> str:
        ...
        if not disabled:
            return "Hayır komutlar geçersizdir!"

        result = ""
        for cmd in disabled:
            result += " - `{}`\n".format(escape_markdown(cmd))
        return "Aşağıdaki komutlar şu anda kısıtlanmıştır:\n{}".format(
            result)
    ...
    def __stats__():
        return f"• {sql.num_disabled()} devre dışı bırakılan öğeler {sql.num_chats()} sohbetlerinde."
    ...
    __help__ = """
    • `/cmds`*:* devre dışı bırakılmış komutların mevcut durumunu kontrol edin

    *Yalnızca yöneticiler:*
    • `/enable <cmd name>`*:* bu komutu etkinleştir
    • `/disable <cmd name>`*:* bu komutu devre dışı bırak
    • `/enablemodule <module name>`*:* o modüldeki tüm komutları etkinleştir
    • `/disablemodule <module name>`*:* o modüldeki tüm komutları devre dışı bırak
    • `/listcmds`*:* olası tüm değiştirilebilir komutları listeler
    """

    __mod_name__ = "Komut devre dışı bırakılıyor"
