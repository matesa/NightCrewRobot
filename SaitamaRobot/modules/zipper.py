...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("Sıkıştırmak için dosyayı yanıtlayın.")
        return 
    if event.is_group:
     if not (await is_register_admin(event.input_chat, event.message.sender_id)):
       await event.reply("Hai .. Sen yönetici değilsin .. Bu komutu kullanamazsın .. Ama benim pm'de kullanabilirsin")
       return
   
    mone = await event.reply("`⏳️Lütfen bekleyin...`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.reply("Şimdi sıkıştırılıyor")
