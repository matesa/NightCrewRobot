...
def broadcast(update: Update, context: CallbackContext):
    ...
    if len(to_send) >= 2:
        ...
        update.effective_message.reply_text(
            f"Yayın tamamlandı.\nGruplar başarısız oldu: {failed}.\nKullanıcılar başarısız oldu: {failed_user}."
        )
...
def chats(update: Update, context: CallbackContext):
    ...
    chatfile = 'Sohbet listesi.\n0. Sohbet adı | Sohbet Kimliği | Üye sayısı\n'
    P = 1
    ...
    with BytesIO(str.encode(chatfile)) as output:
        ...
        update.effective_message.reply_document(
            ...
            caption="Veritabanımdaki grupların listesi burada.")
...
def __user_info__(user_id):
    if user_id in [777000, 1087968824]:
        return """╘══「 Grup sayısı: <code>???</code> 」"""
    if user_id == dispatcher.bot.id:
        return """╘══「 Grup sayısı: <code>???</code> 」"""
    num_chats = sql.get_user_num_chats(user_id)
    return f"""╘══「 Grup sayısı: <code>{num_chats}</code> 」"""
