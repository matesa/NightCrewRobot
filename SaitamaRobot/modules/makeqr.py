...
def progress(current, total):
    """ İndirme ilerlemesini hesaplayın ve verilen bağımsız değişkenlerle geri dönün. """
    print(
        "İndirildi {} of {}\nTamamlandı {}".format(
...
async def _(event):
    ...
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                "Hai .. Sen yönetici değilsin .. Bu komutu kullanamazsın .. Ama benim pm'de kullanabilirsin"
            )
            return
...
async def img_sampler(event):
    ...
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(".. Yönetici değilsin .. bot pm'de kullan")
            return
...
async def parseqr(qr_e):
    """ Getqr komutu için, yanıtlanan fotoğraftan QR Kodu içeriğini alın. """
    if qr_e.fwd_from:
        return
    start = datetime.now()
    ...
    await qr_e.reply(
        "QRCode içeriği {} saniye içinde alındı.\n{}".format(duration, qr_contents)
    )
...
&margin=1&qzone=0&format=jpg"
    ...
    await qrcode.reply("{} saniye içinde QRCode oluşturuldu".format(duration))
    await sleep(5)
