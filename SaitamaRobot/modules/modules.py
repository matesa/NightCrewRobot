...
def load(update: Update, context: CallbackContext):
    ...
    load_messasge = message.reply_text(
        f"Modül yüklenmeye çalışılıyor : <b>{text}</b>", parse_mode=ParseMode.HTML)
    ...
    if not imported_module.__mod_name__.lower() in IMPORTED:
        ...
        load_messasge.edit_text("Modül zaten yüklendi.")
        return
    if "__handlers__" in dir(imported_module):
        ...
        load_messasge.edit_text("Modül yüklenemez.")
        return
    ...
    load_messasge.edit_text(
        "Başarıyla yüklendi modül : <b>{}</b>".format(text),
...
def unload(update: Update, context: CallbackContext):
    ...
    unload_messasge = message.reply_text(
        f"Modül kaldırılmaya çalışılıyor : <b>{text}</b>",
        parse_mode=ParseMode.HTML)
    ...
    if imported_module.__mod_name__.lower() in IMPORTED:
        ...
        unload_messasge.edit_text("Yüklenmemiş bir şey kaldırılamaz.")
        return
    if "__handlers__" in dir(imported_module):
        handlers = imported_module.__handlers__
        for handler in handlers:
            if isinstance(handler, bool):
                unload_messasge.edit_text("Bu modül kaldırılamaz!")
                return
        ...
        unload_messasge.edit_text("Modül kaldırılamaz.")
        return
    ...
    unload_messasge.edit_text(
        f"Başarıyla kaldırılan modül : <b>{text}</b>",
        parse_mode=ParseMode.HTML)
