...
def get_invalid_chats(update: Update,
    ...
    for chat in chats:
        ...
        if ((100 * chats.index(chat)) / len(chats)) > progress:
            progress_bar = f"{progress}% geçersiz sohbetler almada tamamlandı."
            if progress_message:
                try:
...
def dbcleanup(update: Update, context: CallbackContext):
    ...
    reply = f"Toplam geçersiz sohbetler - {invalid_chat_count}\n"
    reply += f"Toplam gbanned kullanıcıları geçersiz - {invalid_gban_count}"
...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat":
        if query.from_user.id in admin_list:
            bot.editMessageText("Sohbetlerden ayrılıyor ...", chat_id,
                                message.message_id)
