...
def debug(update: Update, context: CallbackContext):
    ...
    if len(args) > 1:
        if args[1] in ('yes', 'on'):
            DEBUG_MODE = True
            message.reply_text("Hata ayıklama modu artık açık.")
        elif args[1] in ('no', 'off'):
            DEBUG_MODE = False
            message.reply_text("Hata ayıklama modu artık kapalı.")
    else:
        if DEBUG_MODE:
            message.reply_text("Hata ayıklama modu şu anda açık.")
        else:
            message.reply_text("Hata ayıklama modu şu anda kapalı.")
